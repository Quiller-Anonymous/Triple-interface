...
/--
Variant of `norm_expSum_sub_muMainTerm_le_of_BMOR` with no `Xmin ≤ L-1` cutoff.

This uses the coarse fallback bound from Step 4 (constant `210`), so it is intended only as a
certificate-friendly “no low-end cutoff” option.
-/
theorem norm_expSum_sub_muMainTerm_le_of_BMOR210
    {q a : ℕ}
  ...
  have hrewrite :
    ...
    calc
      ...
      _ = (Goldbach.Cert.SiegelWalfisz.ΛVM n : ℂ) * gExp β n * e ((a : ℝ) * (n : ℝ) / q) := by
          simp [mul_assoc]
  have h :=
    (Goldbach.Cert.MajorArcStep4FullExpSumBMOR.norm_fullExpSum_sub_muMainTerm_le_of_BMOR210
