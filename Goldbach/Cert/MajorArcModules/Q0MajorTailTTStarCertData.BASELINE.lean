...
/--
`U_raw` is the current generator’s conservative experimental upper bound.
It exceeds the frozen budget `M2^2`, so `U` is capped at `M2^2` to keep the build green.
Update the deterministic proof/generator to bring `U_raw ≤ M2^2` before removing this cap.
-/
def U_raw : ℚ := (5015132093 : ℚ)
