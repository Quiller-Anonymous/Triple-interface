...
/--
Variant of `norm_sum_coprimeResidues_sub_mainTerm_le_of_BMOR` with no `Xmin ≤ L-1` cutoff.

This uses Step 2’s coarse fallback bound (constant `210`), so it is intended only as a
certificate-friendly “no low-end cutoff” option.
-/
theorem norm_sum_coprimeResidues_sub_mainTerm_le_of_BMOR210
  ...
  have hbound_each :
    ...
    have hcop : Nat.Coprime r q := (Finset.mem_filter.mp hr).2
    have hstep :=
      Goldbach.Cert.MajorArcStep2ExpSumsBMOR.norm_sum_aTerm_gExp_Ico_sub_mainTerm_le_of_BMOR210
        (q := q) (a := r) (L := L) (U := U) hLU hL hL2 hq hqQ0 hcop (β := β) hβ
    simpa [Sres, V, hc_def, one_div] using hstep
  ...
  have hnonneg :
      0 ≤
        (((210 : ℝ) * (U : ℝ) / Real.log ((L - 1 : ℕ) : ℝ))
          * (2 * (2 + ((U - (L - 1) : ℕ) : ℝ) * (4 * Real.pi * |β|)))) := by
    have hlog_pos : 0 < Real.log ((L - 1 : ℕ) : ℝ) := by
      have h2' : (2 : ℝ) ≤ ((L - 1 : ℕ) : ℝ) := by exact_mod_cast hL2
      have h1 : (1 : ℝ) < ((L - 1 : ℕ) : ℝ) := lt_of_lt_of_le (by norm_num : (1 : ℝ) < 2) h2'
      exact Real.log_pos h1
    have hfrac_nonneg : 0 ≤ (210 : ℝ) * (U : ℝ) / Real.log ((L - 1 : ℕ) : ℝ) :=
      div_nonneg (mul_nonneg (by norm_num) (by positivity)) (le_of_lt hlog_pos)
    have hpi : 0 ≤ (4 * Real.pi * |β|) := by
      have h4 : 0 ≤ (4 : ℝ) := by norm_num
      exact mul_nonneg (mul_nonneg h4 (le_of_lt Real.pi_pos)) (abs_nonneg β)
    have : 0 ≤ (2 : ℝ) + ((U - (L - 1) : ℕ) : ℝ) * (4 * Real.pi * |β|) := by
  ...
  have hfinal :
      ...
      (q : ℝ) *
        (((210 : ℝ) * (U : ℝ) / Real.log ((L - 1 : ℕ) : ℝ))
          * (2 * (2 + ((U - (L - 1) : ℕ) : ℝ) * (4 * Real.pi * |β|)))) := by
    have := le_trans hmain (mul_le_mul_of_nonneg_right hcard_le hnonneg)
    simpa [mul_assoc] using this
