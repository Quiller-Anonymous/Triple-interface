...
/--
Variant of `norm_fullExpSum_sub_muMainTerm_le_of_BMOR` with no `Xmin ≤ L-1` cutoff.

This uses the coarse fallback bound from Step 3 (constant `210`), so it is intended only as a
certificate-friendly “no low-end cutoff” option.
-/
theorem norm_fullExpSum_sub_muMainTerm_le_of_BMOR210
    {q a : ℕ}
  ...
  have hcop_bound :=
    (Goldbach.Cert.MajorArcStep3MajorArcDecompMuBMOR.norm_sum_coprimeResidues_sub_muMainTerm_le_of_BMOR210
