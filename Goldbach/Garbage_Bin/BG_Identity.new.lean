/-
  Goldbach/BG_Identity.lean  —  header + robust helpers (first 120 lines)

  Notes (cf. DontHassleMe):
  • We avoid relying on mathlib lemma names that drift (e.g. abs/sum triangles),
    and we provide small local lemmas with stable statements.
  • No dependence on AO/SingularSeries yet (prevents pipeline cycles).
  • Keep everything in ℝ early to dodge ℤ/ℕ/ℝ cast friction; integer band
    combinatorics will come later in the file.
-/

import Mathlib.Data.Real.Basic
import Mathlib.Topology.Algebra.Algebra  -- conservative; stable in mathlib4
...
/-! ###########################################################################
     Local helper lemmas (stable across mathlib bumps)
############################################################################ -/

/-- Triangle inequality for finite sums in `ℝ`:
`|∑ x∈s, f x| ≤ ∑ x∈s, |f x|`.  We keep this locally to avoid name drift. -/
lemma abs_sum_le_sum_abs (s : Finset α) (f : α → ℝ) :
  ...
  · simp
  · intro a s ha ih
    -- `sum_insert` puts the head on the left; then two-step bound:
    -- 1) triangle for `|f a + Σ_s f|`
    -- 2) plug the induction hypothesis for the tail
    rw [Finset.sum_insert ha, Finset.sum_insert ha]
    have h₁ : |f a + ∑ x in s, f x| ≤ |f a| + |∑ x in s, f x| := abs_add (f a) (∑ x in s, f x)
    have h₂ : |f a| + |∑ x in s, f x| ≤ |f a| + ∑ x in s, |f x| := add_le_add_left ih _
    exact h₁.trans h₂

/-- A tiny helper for the very common pattern
`a ≤ b` with `0 < b` gives `a / b ≤ 1`.  This stabilizes the old usage
`(div_le_iff₀ hbpos).mpr h` that expects `a ≤ 1 * b`. -/
lemma div_le_one_of_le (a b : ℝ) (hbpos : 0 < b) (h : a ≤ b) : a / b ≤ 1 := by
  -- make the expected RHS `1*b` explicit
  have h' : a ≤ 1 * b := by simpa [one_mul] using h
  simpa using (div_le_iff₀ hbpos).mpr h'

/-- Pointwise capping lemma:
If `|a k| ≤ C` on `s`, then `∑ |a k|*|b k| ≤ C * ∑ |b k|`.  We prove it by
pointwise monotonicity and a clean factorization, avoiding fragile `simp` names.
-/
lemma sum_abs_mul_le_cap
  ...
  classical
  -- 1) pointwise: `|a k|*|b k| ≤ C*|b k|` (since `|b k| ≥ 0`)
  have hpt : ∀ k ∈ s, |a k| * |b k| ≤ C * |b k| := by
    intro k hk
    have hb : 0 ≤ |b k| := abs_nonneg _
    exact mul_le_mul_of_nonneg_right (hCap k hk) hb
  -- 2) sum the pointwise inequality over `s`
  have hsum : ∑ k in s, |a k| * |b k| ≤ ∑ k in s, C * |b k| :=
    Finset.sum_le_sum hpt
  -- 3) factor out `C` from the RHS sum without relying on lemma-name drift
  --    (we give the very small induction explicitly).
  have factor :
      ∑ k in s, C * |b k| = C * ∑ k in s, |b k| := by
    refine Finset.induction_on s ?z ?step
    · simp
    · intro k s hk ih
      simp [hk, ih, mul_add, add_comm, add_left_comm, add_assoc,
            mul_comm, mul_left_comm, mul_assoc]
  -- 4) conclude
  simpa [factor]

/-- A simple corollary that often reads better at call sites:
`|∑ a k * b k| ≤ C * ∑ |b k|` assuming `|a k| ≤ C` on `s`.  This packages
`abs_sum_le_sum_abs` and `sum_abs_mul_le_cap`. -/
lemma abs_sum_mul_le_cap_sum_abs
    (s : Finset α) (a b : α → ℝ) (C : ℝ)
    (hCap : ∀ k ∈ s, |a k| ≤ C) :
    |∑ k in s, a k * b k| ≤ C * ∑ k in s, |b k| := by
  classical
  -- Triangle inequality on the sum of products:
  have tri :
      |∑ k in s, a k * b k| ≤ ∑ k in s, |a k * b k| :=
    abs_sum_le_sum_abs s (fun k => a k * b k)
  -- Make the middle term match the capped lemma:
  have hrewrite :
      ∑ k in s, |a k * b k| = ∑ k in s, |a k| * |b k| := by
    simp [abs_mul]
  have tri' :
      |∑ k in s, a k * b k| ≤ ∑ k in s, |a k| * |b k| := by
    simpa [hrewrite] using tri
  -- Apply the pointwise cap and sum:
  have base :
      ∑ k in s, |a k| * |b k| ≤ C * ∑ k in s, |b k| :=
...
lemma K_full_peak_le (U : ℕ) (k : ℤ) : |K_full_raw U k| ≤ (U : ℝ)⁻¹ := by
  ...
  · have hUpos : 0 < (U : ℝ) := by exact_mod_cast (Nat.pos_of_ne_zero hU)
    have hUpos' : 0 ≤ (U : ℝ) := le_of_lt hUpos
    have hmax_le_one :
        max (1 - |(k : ℝ)| / (U : ℝ)) 0 ≤ 1 := by
      have hsub_le : 1 - |(k : ℝ)| / (U : ℝ) ≤ 1 := by
        have hnonneg : 0 ≤ |(k : ℝ)| / (U : ℝ) :=
          div_nonneg (abs_nonneg _) hUpos'
        linarith
      exact max_le_iff.mpr ⟨hsub_le, by linarith⟩
    have hmax_nonneg :
        0 ≤ max (1 - |(k : ℝ)| / (U : ℝ)) 0 := by
      exact le_max_right _ _
    have hnonneg :
        0 ≤ max (1 - |(k : ℝ)| / (U : ℝ)) 0 / (U : ℝ) := by
      exact div_nonneg hmax_nonneg hUpos'
    have hdiv :
        max (1 - |(k : ℝ)| / (U : ℝ)) 0 / (U : ℝ) ≤ 1 / (U : ℝ) := by
      have := div_le_div_of_nonneg_right hmax_le_one hUpos'
    ...
    unfold K_full_raw
    simpa [habs, one_div] using hdiv

/-- Evenness of the tent kernel. -/
lemma K_full_neg (k : ℤ) : K_full (-k) = K_full k := by
  unfold K_full K_full_raw
  have habs : |(-k : ℝ)| = |(k : ℝ)| := by simpa using abs_neg (k : ℝ)
  simp [habs]

/-- The unnormalized full tent is nonnegative. -/
lemma tentFullWeight_nonneg (k : ℤ) : 0 ≤ tentFullWeight k := by
  classical
  unfold tentFullWeight K_full
  have hU : 0 ≤ (Ucut : ℝ) := by exact_mod_cast (Nat.zero_le Ucut)
  -- max term is always ≥ 0
  have hmax : 0 ≤ max (1 - |(k : ℝ)| / (Ucut : ℝ)) 0 := by
    exact le_max_right _ _
  -- dividing by nonnegative preserves nonnegativity
...
lemma K_full_inner_peak_le {k : ℤ} (hk : k ∈ S_BG) :
  ...
  have hkNatAbs : Int.natAbs k ≤ H := by
    ...
    have hkabs : |k| ≤ (H:ℤ) := abs_le.mpr hk'
  ...
  have habs_eq : |(k : ℝ)| = (Int.natAbs k : ℝ) := by
    rw [abs_eq_natAbs, Int.cast_natAbs]
  have hterm : (1 - (Int.natAbs k : ℝ) / (Ucut : ℝ)) / (Ucut : ℝ)
        ≤ 1 / (Ucut : ℝ) := by
    nlinarith
  have hmax_le : max (1 - |(k : ℝ)| / (Ucut : ℝ)) 0 ≤ 1 := by
    apply max_le
    · have : |(k : ℝ)| / (Ucut : ℝ) ≥ 0 := div_nonneg (abs_nonneg _) (le_of_lt hUpos)
      linarith
    · linarith
  have hmax_nonneg : 0 ≤ max (1 - |(k : ℝ)| / (Ucut : ℝ)) 0 := le_max_right _ _
  have hdiv_le : max (1 - |(k : ℝ)| / (Ucut : ℝ)) 0 / (Ucut : ℝ) ≤ 1 / (Ucut : ℝ) := by
    exact div_le_div_of_nonneg_right hmax_le (le_of_lt hUpos)
  exact hdiv_le

/-- Absolute tent mass on the inner band: ∑ |K_full| ≤ (2H+1)/Ucut. -/
lemma sum_abs_K_full_inner_le :
    Finset.sum S_BG (fun k => |K_full k|) ≤ ((2*H+1 : ℕ) : ℝ) / (Ucut : ℝ) := by
  ...
  calc
    Finset.sum S_BG (fun k => |K_full k|)
        ≤ Finset.sum S_BG (fun _ => (1 / (Ucut : ℝ))) := by
...
lemma swap_bound_linf_l1
  ...
  have h1 :
      |Finset.sum S_BG (fun k => K_full k * (P k - Q k))|
        ≤ Finset.sum S_BG (fun k => |K_full k * (P k - Q k)|) := by
    simpa using Finset.abs_sum_le_sum_abs (s:=S_BG)
      (f:=fun k => K_full k * (P k - Q k))
  have h2 :
      Finset.sum S_BG (fun k => |K_full k * (P k - Q k)|)
        ≤ Finset.sum S_BG (fun k => |K_full k| * M) := by
    refine Finset.sum_le_sum ?_; intro k hk
    have hPm := hM k hk
    nlinarith [abs_mul, abs_nonneg (K_full k), hPm]
  have hsum : Finset.sum S_BG (fun k => |K_full k|) ≤ ((2*H+1 : ℕ) : ℝ) / (Ucut : ℝ) :=
    sum_abs_K_full_inner_le
  have h3 :
      Finset.sum S_BG (fun k => |K_full k| * M)
        = (Finset.sum S_BG (fun k => |K_full k|)) * M := by ring
  calc
    |Finset.sum S_BG (fun k => K_full k * (P k - Q k))|
        ≤ (Finset.sum S_BG (fun k => |K_full k|)) * M := by
          have := le_trans h1 h2
...
lemma innerK_card_le_twoHplus1 (H : ℕ) :
  ...
  have hfilter : ((Finset.filter (fun k : ℤ => abs k ≤ (H:ℤ))
...
lemma ppInnerCount_le_two_mul_innerSquares
  ...
  exact sorry  -- replace with your local proof; it’s short!
...
lemma ppContam_le_canon {X N : ℕ}
  ...
  have hNX : X ≤ N := by
    ...
    rcases Finset.mem_image.mp hwin with ⟨k, hk, rfl⟩
    exact Nat.le_add_right X k
  have hN_ge_X0 : BankParams.X0 ≤ N := le_trans hX hNX
  have hpp16 : ppInnerCount BankParams.H N ≤ 16 := by
    simpa using (ppInnerCount_le_16 (N := N) hN_ge_X0)
  have h : (ppInnerCount BankParams.H N : ℝ) ≤ 16 := by
    exact_mod_cast hpp16
  have hCpp : (16 : ℝ) ≤ Cpp_canon := by decide
  exact le_trans h hCpp
...
/-- Reference tent weight: the full tent kernel, but cut off outside the inner band `S_BG`. -/
...
lemma sum_pos_of_pos_at_zero :
  ...
  have hrest : 0 ≤ Finset.sum (S_BG.erase 0) (fun k => K_full k) := by
    exact Finset.sum_nonneg hnonneg
  have hsplit :
      Finset.sum S_BG (fun k => K_full k) =
        K_full (0 : ℤ) + Finset.sum (S_BG.erase 0) (fun k => K_full k) := by
    exact Finset.sum_erase_add (s := S_BG) (a := 0) hk0
  have hadd : 0 < K_full (0 : ℤ) + Finset.sum (S_BG.erase 0) (fun k => K_full k) :=
    add_pos_of_pos_of_nonneg hpos0 hrest
  exact (hsplit.symm ▸ hadd)

/-- Reference in-window operator: equals the main term on the window, conv_ref off it. -/
noncomputable def bankOp_ref (X N : ℕ) : ℝ :=
  if h : BankParams.X0 ≤ X ∧ N ∈ Windows.EvenIn X BankParams.H then
...
lemma bankOp_full_eq_R_on_window {X N : ℕ}
    (hX : BankParams.X0 ≤ X) (hN : N ∈ Windows.EvenIn X BankParams.H) :
    bankOp_full X N = (Goldbach.Rep.R N : ℝ) := by
  have h : BankParams.X0 ≤ X ∧ N ∈ Windows.EvenIn X BankParams.H := ⟨hX, hN⟩
  simp [bankOp_full, h]

/-- Calibration on the window: reference bank operator reproduces `M`. -/
lemma bankOp_ref_eq_M_on_window {X N : ℕ}
    (hX : BankParams.X0 ≤ X) (hN : N ∈ Windows.EvenIn X BankParams.H) :
...
lemma mass_BG_pos : 0 < mass_BG := by
  simpa [mass_BG] using sum_pos_of_pos_at_zero

/-- Constant reference payload: sigma·weight_mass divided by kernel mass on the window. -/
noncomputable def Pref (X N : ℕ) (k : ℤ) : ℝ :=
  (SingularSeries.sigma Goldbach.Analytic.C2_numeric N * AO_Major.weight_mass X) / mass_BG
...
lemma conv_ref_const_eq_sigma_mass (X N : ℕ) :
    conv_ref_const X N =
      SingularSeries.sigma Goldbach.Analytic.C2_numeric N * AO_Major.weight_mass X := by
  classical
  unfold conv_ref_const Pref mass_BG
  -- factor out the constant payload
  have hfactor :
      (Finset.sum S_BG (fun k =>
          ((SingularSeries.sigma Goldbach.Analytic.C2_numeric N * AO_Major.weight_mass X)
            / Finset.sum S_BG (fun k => K_full k)) * K_full k)) =
        ((SingularSeries.sigma Goldbach.Analytic.C2_numeric N * AO_Major.weight_mass X)
          / Finset.sum S_BG (fun k => K_full k)) *
    ...
    simpa using
      (Finset.mul_sum
        ((SingularSeries.sigma Goldbach.Analytic.C2_numeric N * AO_Major.weight_mass X)
          / Finset.sum S_BG (fun k => K_full k))
        (fun k => K_full k) S_BG).symm
  -- cancel the kernel mass (it is positive)
  have hne : (Finset.sum S_BG (fun k => K_full k) : ℝ) ≠ 0 := by
    exact ne_of_gt sum_pos_of_pos_at_zero
  -- finish
  simpa [hfactor, div_eq_mul_inv, mul_assoc, mul_left_comm, mul_comm, hne] using
    (show
      (((SingularSeries.sigma Goldbach.Analytic.C2_numeric N * AO_Major.weight_mass X)
        / Finset.sum S_BG (fun k => K_full k)) * Finset.sum S_BG (fun k => K_full k)) =
        (SingularSeries.sigma Goldbach.Analytic.C2_numeric N * AO_Major.weight_mass X) from by
          simp [div_eq_mul_inv, hne, mul_assoc, mul_left_comm, mul_comm])
...
lemma tail_mass_le_bound :
  ...
  classical
  have hmax :
      Finset.sum outerBand (fun k => K_full k)≤ (outerBand.card : ℝ) * ((1 - ((H+1 : ℝ) / (Ucut : ℝ))) / (Ucut : ℝ)) := by
...
lemma errTI_bound_from_tail
  ...
  have step1 :
      |Finset.sum outerBand (fun k => P_BG X N k * K_full k)|
        ≤ payload_cap N * Finset.sum outerBand (fun k => |K_full k|) := by
...
lemma sum_innerBand :
  ...
  have hsplit :
    ...
    have hband : (Finset.Icc (-(H:ℤ)) (H:ℤ)).erase 0
      ...
      have hpos_sum :
          Finset.sum ((Finset.Icc (-(H:ℤ)) (H:ℤ)).erase 0) (fun k => K_full k)
            = 2 * Finset.sum (Finset.range H) (fun k => K_full (Int.ofNat (Nat.succ k))) := by
        have hpair :
      ...
      calc
        Finset.sum innerBand (fun k => K_full k)
            = K_full 0
              + Finset.sum ((Finset.Icc (-(H:ℤ)) (H:ℤ)).erase 0) (fun k => K_full k) := by
  ...
  have hsumk : Finset.sum (Finset.range H) (fun k => (Nat.succ k : ℝ)) = (H * (H + 1) : ℝ) / 2 := by
    ...
    have hsum_nat : Finset.sum (Finset.range H) (fun k => Nat.succ k) = H * (H + 1) / 2 := by
      ...
      -- drop the last term
      have hsplit :
          Finset.sum (Finset.range (Nat.succ H)) (fun i => i)
  ...
  have hpos_eval :
    ...
    calc
      Finset.sum (Finset.range H) (fun k => K_full (Int.ofNat (Nat.succ k)))
          = Finset.sum (Finset.range H) (fun k => ((1 : ℝ) - (Nat.succ k : ℝ) / (Ucut : ℝ))) / (Ucut : ℝ) := by
            apply Finset.sum_congr rfl
            intro k hk
            have hk' : Nat.succ k ≤ Ucut := by
      ...
      _ = ((H : ℝ) - ((H * (H + 1) : ℝ) / (Ucut : ℝ))) / (Ucut : ℝ) := by
        field_simp [hUpos] ; ring
  calc
    Finset.sum innerBand (fun k => K_full k)= (1 : ℝ) / (Ucut : ℝ) + 2 * (((H : ℝ) - ((H * (H + 1) : ℝ) / (Ucut : ℝ))) / (Ucut : ℝ)) := by
