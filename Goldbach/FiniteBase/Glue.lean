...
lemma union_even_adjacent {a b c : ℕ}
  ...
  ·
    ...
    have hb1_ne : n ≠ b + 1 := by
      intro h
      have h_even_succ : Even (b + 1) := by simpa [h] using hn
      have hnot_even_b : ¬ Even b := Nat.even_add_one.mp h_even_succ
      have h_odd_b : Odd b := Nat.not_even_iff_odd.mp hnot_even_b
      exact (Nat.not_odd_iff_even.2 hb_even) h_odd_b
    have hb1_lt : b + 1 < n := lt_of_le_of_ne hb1_le hb1_ne.symm
    have hb2_le : b + 2 ≤ n := Nat.succ_le_of_lt hb1_lt
    exact h₂ (N := n) hn hb2_le hnc
