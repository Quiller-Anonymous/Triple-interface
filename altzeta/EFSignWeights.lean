...
def fejerDelta (bp : BandParams) (z : ℂ) : ℝ :=
  (1 - bp.α) * fejerAlignmentKappa bp * Complex.abs z
...
lemma complex_ne_zero_of_pos_re {z : ℂ} (hz : 0 < z.re) : z ≠ 0 := by
  intro hzero
  have : z.re = 0 := by simpa [hzero]
  exact (lt_irrefl _ (show z.re < z.re from lt_of_le_of_lt (le_of_eq this) hz))

/-- Absolute value is positive when the real part is positive. -/
lemma complex_abs_pos_of_pos_re {z : ℂ} (hz : 0 < z.re) : 0 < Complex.abs z := by
  have hne : z ≠ 0 := complex_ne_zero_of_pos_re hz
  exact Complex.abs.pos.mpr hne
...
lemma delta_pos (hz : OffLineZeroWitness setup) : 0 < fc.delta hz := by
  ...
  have hκ := fc.hCos
  have habs : 0 < Complex.abs hz.s := complex_abs_pos_of_pos_re hposRe
