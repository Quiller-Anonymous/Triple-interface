...
`Twin.MajorArcPin.gate_onWindow`; the analytic hypotheses are not yet used.
This keeps the project honest (no new axioms) while giving a stable target
signature for the eventual proof. -/
theorem gate_onWindow_of_SME_CLS
  {A B : ℝ} {Λ : ℕ → ℝ} {W W_hat : ℝ → ℝ}
  (P : GoalAPI.Params)
  (emin eds : ℕ → ℝ)
  (_hCLS  : AnalyticCore.CLSBound P emin)
  (_hDesm : AnalyticCore.DesmoothBound P eds)
  (_sme   : SmoothMajorArcEstimate A B Λ W W_hat)
  (_spec  : GateSpec P) :
...
by
  ...
  exact Twin.MajorArcPin.gate_onWindow P emin eds
